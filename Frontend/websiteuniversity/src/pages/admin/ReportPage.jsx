                  ...
                  {filtered.map((r) => (
                    <tr key={r.id} style={!r.read ? { background: "#FBF7EE" } : undefined}>
                      <td style={{ whiteSpace: "nowrap" }}>
                        ...
                        <div style={{ color: "#3E5EDB", fontSize: 11.5, fontWeight: 700 }}>{r.studentId || ""}</div>
                      </td>
                      <td><span className="type-pill" style={{ background: typeColor(r.type) }}>{r.type || t("Other")}</span></td>
                      <td>
                        <div className="rp-report-title">{r.title || "-"}</div>
                        <div className="rp-report-body">{r.content || r.details || ""}</div>
                      </td>
